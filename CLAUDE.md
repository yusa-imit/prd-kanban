...
### Phase 3: 브라우저 검증 및 마일스톤 확인

구현이 완료되면 **브라우저에서 실제 동작을 확인**한 뒤, PRD의 마일스톤 기준 체크리스트를 하나씩 검증하고 업데이트한다.

1. `vp dev`로 개발 서버를 실행하고 브라우저에서 결과물을 직접 확인한다.
2. PRD 마일스톤 체크리스트의 각 항목을 브라우저에서 검증한다.
3. 검증을 통과한 항목은 `- [x]`로 체크하고, 미충족 항목은 `- [ ]`로 유지한다.
4. 미충족 항목이 있으면 해당 부분을 수정하고 다시 브라우저에서 검증한다.
5. 모든 체크리스트 항목이 `[x]`가 되면 해당 이터레이션을 완료로 간주한다.
6. 마일스톤 달성 후 다음 이터레이션의 PRD 작성으로 넘어간다.
